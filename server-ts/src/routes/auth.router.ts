...
class AuthController {
  ...
  private initRoutes () {
    this.router.post(
			...
			[
				...
				check('password', 'Minimum length is 7 ').isLength({min: 7})
			],
			 this.register
		);
		this.router.post(
  ...
  private async register (req: express.Request, res: express.Response){
      try {
          console.log(req.body);
					...
					if (!errors.isEmpty()){
						return res.status(400).send({errors: errors.array, message: 'Incorrect date for register'});
					}
  ...
  private async login (req: express.Request, res: express.Response) {
      ...
      console.log(req.body);
			const errors  = validationResult(req);
			if (!errors.isEmpty()){
				return res.status(400).send({errors: errors.array, message: 'Inccorect data for login'});
			}
